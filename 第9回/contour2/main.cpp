...
int main(int argc, const char * argv[]) {
    ...
    cv::threshold(gray_img, bin_img, BIN_TH, 255, cv::THRESH_BINARY_INV);
    ...
    //8.3 領域特微量の取得・表示
    double L,S;
    L = cv::arcLength(contours[num], true);
    S = cv::contourArea(contours[num]);
    printf("周囲長；%lf, 面積：%lf\n",L,S);
    
    //8.4 外接長方形の取得
    cv::Rect bb = cv::boundingRect(contours[num]);
