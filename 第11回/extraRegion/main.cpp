...
//            p = src_img.at<cv::Vec3b>(y, x); //各画素のRGB値を取得
//            b = p[0]; g = p[1]; r = p[2];
//            if (r>=R_MIN && G_MIN<=g && g<=G_MAX && B_MIN<=b && b<=B_MAX) {
//                dst_img.at<unsigned char>(y,x) = WHITE_PIXEL;
//            }
//        }
//    }
    
    cv::inRange(src_img, cv::Scalar(B_MIN, G_MIN, R_MIN), cv::Scalar(B_MAX, G_MAX, 255), dst_img);
